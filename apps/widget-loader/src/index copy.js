  ...
  const loadImportmap = () =>
    new Promise((resolve, reject) => {
      console.log(
        ...
        document.querySelector(`script[data-juno-importmap="${origin}"]`)
      )
  ...
  loadShim()
    ...
    .then(async () => {
      ...
      if (!appURL) {
        ...
        // widget loader
        if (
          !window.__junoWidgetLoader.originManifests[origin] ||
          !window.__junoWidgetLoader.originManifests[origin].manifest
        ) {
        ...
        if (
          debug &&
          (!window.__junoWidgetLoader.originManifests[origin].manifest ||
            !window.__junoWidgetLoader.originManifests[origin].manifest[name] ||
            !window.__junoWidgetLoader.originManifests[origin].manifest[name][
              version
            ])
        ) {
