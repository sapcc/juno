...
const build = async () => {
  ...
  // copy wasm file to build folder once without watching
  await esbuild.build({
    entryPoints: ["@swc/wasm-web/wasm-web_bg.wasm"],
    outfile: `${outdir}/wasm-web_bg.wasm`,
    bundle: true,
    platform: "browser",
    // built-in loaders: js, jsx, ts, tsx, css, json, text, base64, dataurl, file, copy, binary
    loader: { ".wasm": "copy" },
  })
